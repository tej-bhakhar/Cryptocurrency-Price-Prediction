...
class ProfitCalculator:
    ...
    def create_dataframe(self):
        low = np.array(self.predicted_low).flatten()
        high = np.array(self.predicted_high).flatten()
        mean = np.array(self.mean_prediction).flatten()
        if not (len(low) == len(high) == len(mean)):
            raise ValueError(f"Prediction lengths don't match: "
                            f"low={len(low)}, high={len(high)}, mean={len(mean)}")
        arr = np.column_stack((low, high, mean))
        ...
        signal = pd.DataFrame(np.column_stack((s1, s2)), columns=['signal1', 'signal2'])
